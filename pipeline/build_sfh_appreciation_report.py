...
def risks(state: str, yoy: float) -> str:
    ...
    if not rs:
        rs.append("block-level variance; confirm local supply pipeline")
    return "; ".join(rs)
...
def main() -> None:
    ...
    add("| | |")
    add("|---|---|")
    add("| [1. What changed](#1-what-changed-vs-the-prior-run) | [2. National snapshot](#2-national-market-snapshot) |")
    add("| [3. Top 10 / lists](#3-top-10-actionable-markets) | [4. All-state matrix](#4-all-state-ranking-matrix) |")
    ...
    add("| [5. City leaderboards](#5-city-leaderboards) | [6. All-state deep dives](#6-all-state-deep-dives) |")
    add("| [7. Legal](#7-legal-environment--verified-2026-highlights) | [8. Insurance & tax](#8-insurance-and-property-tax-overlays) |")
    add("| [9. Property management](#9-property-management-rates--remote-ops) | [10. Acquisition workflow](#10-practical-acquisition-workflow) |")
    add("| [11. Methodology & sources](#11-methodology-and-sources) | [A–Z state rank index](#az-actionable-rank-index) |")
    add("")
    ...
    add("")
    add("**Defaults used (user did not override):** appreciation-first; single-family only; remote-capable preferred; **5–10+ year** hold; moderate risk; thin day-one cash flow acceptable.")
    add("")
    add("---")
    add("")
    ...
    add("[↑ Back to Index](#index)")
    add("")
    add("Tie-breakers after equal economic scores: **exit liquidity**, data confidence, lower insurance catastrophe risk, remote management availability, diversified jobs.")
    add("")
    ...
    add("| 8 | Georgia — Atlanta job pockets | Strong owner law + migration |")
    add("| 9 | North Carolina — Raleigh / Charlotte | Jobs / migration equity thesis |")
    ...
    add("[↑ Back to Index](#index)")
    add("")
    add("Legal scores are reused directionally from the base rental report research layer. For appreciation holds, **owner-law friction matters for ops and exit timing**, even when cash flow is secondary.")
    add("")
    ...
    add("| **California** | Statewide rent cap (5% + inflation, max 10%) + local overlays | Supply-constrained equity thesis ≠ easy ops |")
    add("| **New York** | Good Cause in opt-in cities; strong tenant tilt | Prefer **upstate SFH** for App; NYC specialist-only |")
    add("| **New Jersey** | Anti-Eviction Act; many local rent ordinances | Equity / spillover demand with tax + ops drag |")
    add("| **Illinois** | State preempts rent control; Chicago has strong local L-T ordinance | Chicago App works with city-rule underwriting |")
    ...
    add("[↑ Back to Index](#index)")
    add("")
    add("Lighter section than the base rental report: appreciation investors still need **professional ops** so thin cash flow does not become deferred maintenance.")
    add("")
    ...
    add("")
    add("Prefer metros with **multiple competing local PMs** (Chicago suburbs, Indianapolis, Milwaukee, Dallas–Fort Worth, Atlanta, Phoenix, Raleigh). Interview 2–3 managers; get fee schedules in writing. Institutional SFR landlords (Invitation Homes, Progress Residential, etc.) are **comps / competitors**, not your third-party PM.")
    add("")
    add("---")
    add("")
    ...
    add("5. Get property-tax history and a **bindable insurance quote** before finalizing.")
    ...
    add("10. Verify local licensing, notice, rent-cap, and just-cause rules.")
    add("11. Buy only if the **address-level** equity path still works with reserves intact.")
    add("")
    add("---")
    add("")
    ...
    add("")
    add("Owner / Tenant law scores are shown separately (not in Econ) and reused from the base legal judgment layer.")
    add("")
    ...
    add("| Property management | about **10%** of collected rent unless quoted |")
